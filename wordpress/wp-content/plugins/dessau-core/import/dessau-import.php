...
class DessauCoreImport {
	...
	public function import_content( $file ) {
		ob_start();
		require_once( DESSAU_CORE_ABS_PATH . '/import/class.wordpress-importer.php' );
		$dessau_import = new WP_Import();
		set_time_limit( 0 );
		
		$dessau_import->fetch_attachments = $this->attachments;
		$returned_value                  = $dessau_import->import( $file );
		
		if ( is_wp_error( $returned_value ) ) {
			$this->message = esc_html__( 'An Error Occurred During Import', 'dessau-core' );
		} else {
			$this->message = esc_html__( 'Content imported successfully', 'dessau-core' );
		}
	...
	public function import_widgets( $file, $file2 ) {
		...
		foreach ( (array) $options['widgets'] as $dessau_widget_id => $dessau_widget_data ) {
			update_option( 'widget_' . $dessau_widget_id, $dessau_widget_data );
		}
		
		$this->import_sidebars_widgets( $file );
		$this->message = esc_html__( 'Widgets imported successfully', 'dessau-core' );
	}
	
	public function import_sidebars_widgets( $file ) {
		$dessau_sidebars = get_option( "sidebars_widgets" );
		unset( $dessau_sidebars['array_version'] );
		$data = $this->file_options( $file );
		
		if ( is_array( $data['sidebars'] ) ) {
			$dessau_sidebars = array_merge( (array) $dessau_sidebars, (array) $data['sidebars'] );
			unset( $dessau_sidebars['wp_inactive_widgets'] );
			$dessau_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $dessau_sidebars );
			$dessau_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $dessau_sidebars );
		}
	}
	...
	public function import_custom_sidebars( $file ) {
		...
		$this->message = esc_html__( 'Custom sidebars imported successfully', 'dessau-core' );
	}
	
	public function import_options( $file ) {
		$options       = $this->file_options( $file );
		$result        = update_option( 'qodef_options_dessau', $options );
		$this->message = esc_html__( 'Options imported successfully', 'dessau-core' );
	}
	
	public function import_menus( $file ) {
		global $wpdb;
		$dessau_terms_table = $wpdb->prefix . "terms";
		$this->menus_data  = $this->file_options( $file );
		$menu_array        = array();
		
		foreach ( $this->menus_data as $registered_menu => $menu_slug ) {
			$term_rows = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $dessau_terms_table where slug=%s", $menu_slug ), ARRAY_A );
	...
	public function import_settings_pages( $file ) {
		...
		foreach ( $pages as $dessau_page_option => $dessau_page_id ) {
			update_option( $dessau_page_option, $dessau_page_id );
		}
	}
	
	public function rev_sliders() {
		$rev_sldiers = array(
			'horizontal-showcase.zip',
			'landing-purchase.zip',
			'architecture-studio.zip',
			'decor-studio.zip',
			'architect-portfolio.zip',
			'landing-portfolio.zip'
		);
	...
	function qodef_admin_import() {
		if ( dessau_core_theme_installed() ) {
			...
			$this->pagehook = add_submenu_page(
                SELECT_OPTIONS_SLUG,
				esc_html__( 'Dessau Options - Dessau Import', 'dessau-core' ), // The value used to populate the browser's title bar when the menu page is active
				esc_html__( 'Import', 'dessau-core' ),                     // The text of the menu in the administrator's sidebar
				'administrator',                                          // What roles are able to access the menu
                SELECT_OPTIONS_SLUG . $slug,                  // The ID used to bind submenu items to this menu
				array( $dessau_select_Framework->getSkin(), 'renderImport' )
			);
			
			add_action( 'admin_print_scripts-' . $this->pagehook, 'dessau_select_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $this->pagehook, 'dessau_select_enqueue_admin_styles' );
		}
	}
	...
	function qodef_update_options_after_import($folder) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options = get_option('qodef_options_dessau');
			$new_global_values = str_replace($demo_url, $url, $global_options);
			
			update_option('qodef_options_dessau', $new_global_values);
		}
	}
