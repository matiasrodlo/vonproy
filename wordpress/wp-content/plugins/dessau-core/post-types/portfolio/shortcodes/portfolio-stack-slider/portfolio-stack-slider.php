...
class PortfolioStackSlider implements Lib\ShortcodeInterface {
	private $base;
	
	public function __construct() {
		$this->base = 'qodef_portfolio_stack_slider';
		
		add_action( 'vc_before_init', array( $this, 'vcMap' ) );
		
		//Portfolio category filter
		add_filter( 'vc_autocomplete_qodef_portfolio_stack_slider_category_callback', array( &$this, 'portfolioCategoryAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Portfolio category render
		add_filter( 'vc_autocomplete_qodef_portfolio_stack_slider_category_render', array( &$this, 'portfolioCategoryAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Portfolio selected projects filter
		add_filter( 'vc_autocomplete_qodef_portfolio_stack_slider_selected_projects_callback', array( &$this, 'portfolioIdAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Portfolio selected projects render
		add_filter( 'vc_autocomplete_qodef_portfolio_stack_slider_selected_projects_render', array( &$this, 'portfolioIdAutocompleteRender', ), 10, 1 ); // Render exact portfolio. Must return an array (label,value)
		
		//Portfolio tag filter
		add_filter( 'vc_autocomplete_qodef_portfolio_stack_slider_tag_callback', array( &$this, 'portfolioTagAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Portfolio tag render
		add_filter( 'vc_autocomplete_qodef_portfolio_stack_slider_tag_render', array( &$this, 'portfolioTagAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array
	}
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'     => esc_html__( 'Portfolio Stack Slider', 'dessau-core' ),
					'base'     => $this->base,
					'category' => esc_html__( 'by DESSAU', 'dessau-core' ),
					'icon'     => 'icon-wpb-portfolio-stack-slider extended-custom-icon',
					'category' => esc_html__( 'by DESSAU', 'dessau-core' ),
					'params'   => array(
						array(
							'type'        => 'textfield',
						...
						array(
							...
							'description' => esc_html__( 'Speed of slide animation in milliseconds. Default value is 1000.', 'dessau-core' ),
							'group'       => esc_html__( 'Slider Settings', 'dessau-core' )
						),
						array(
							'type'        => 'dropdown',
	...
	public function render( $atts, $content = null ) {
		...
		$params['type']                = 'gallery';
		$params['number_of_columns']   = 'one';
		$params['enable_excerpt']      = 'no';
		$params['portfolio_slider_on'] = 'no';
		$params['space_between_items'] = 'no';
		
		$html = '<div class="qodef-portfolio-stack-slider-holder">';

		$html .= dessau_select_execute_shortcode( 'qodef_portfolio_list', $params );

		$html .=  '<div class="qodef-info-box qodef-info-box-hidden"><a class="qodef-pli-link-title" href="#"><h2 itemprop="name" class="qodef-pli-title entry-title"></h2></a>';
		$html .= '<div class="qodef-pli-btn-holder"><a class="qodef-btn-plus" href="#"><span class="qodef-btn-line-1"></span><span class="qodef-btn-line-2"></span></a></div>';
		$html .= '</div>';
		$html .=  '<div class="qodef-slider-nav"><div class="qodef-prev"><span class="qodef-prev-icon lnr lnr-chevron-left"></span></div><div class="qodef-next"><span class="qodef-next-icon lnr lnr-chevron-right"></span></div></div>';
		$html .=  '</div>';
