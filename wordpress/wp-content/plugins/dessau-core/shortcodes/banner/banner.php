<?php
namespace DessauCore\CPT\Shortcodes\Banner;
...
class Banner implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Banner', 'dessau-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Image Overlay Color', 'dessau-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'hover_behavior',
							'heading'     => esc_html__( 'Hover Behavior', 'dessau-core' ),
							'value'       => array(
								esc_html__( 'Visible on Hover', 'dessau-core' )   => 'qodef-visible-on-hover',
								esc_html__( 'Visible on Default', 'dessau-core' ) => 'qodef-visible-on-default',
								esc_html__( 'Disabled', 'dessau-core' )           => 'qodef-disabled'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Position', 'dessau-core' ),
							'value'       => array(
								esc_html__( 'Default', 'dessau-core' )  => 'default',
								esc_html__( 'Centered', 'dessau-core' ) => 'centered'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Content Padding', 'dessau-core' ),
							'description' => esc_html__( 'Please insert padding in format top right bottom left', 'dessau-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'subtitle',
							'heading'    => esc_html__( 'Subtitle', 'dessau-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'subtitle_tag',
							'heading'     => esc_html__( 'Subtitle Tag', 'dessau-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Subtitle Color', 'dessau-core' ),
							'dependency' => array( 'element' => 'subtitle', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'heading'    => esc_html__( 'Link Text', 'dessau-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'link_color',
							'heading'    => esc_html__( 'Link Text Color', 'dessau-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'link_top_margin',
							'heading'    => esc_html__( 'Link Text Top Margin (px)', 'dessau-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$html = dessau_core_get_shortcode_module_template_part( 'templates/banner', 'banner', '', $params );
	...
	private function getLinkStyles( $params ) {
		...
		if ( ! empty( $params['link_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . dessau_select_filter_px( $params['link_top_margin'] ) . 'px';
		}
