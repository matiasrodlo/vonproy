<?php
namespace DessauCore\CPT\Shortcodes\Icon;
...
class Icon implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Icon', 'dessau-core' ),
				    'base'                      => $this->base,
					    ...
					    array(
						    array(
							    ...
							    'value'      => array(
								    ...
								    esc_html__( 'Huge', 'dessau-core' )   => 'qodef-icon-huge'
							    )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'custom_size',
							    'heading'    => esc_html__( 'Custom Size (px)', 'dessau-core' )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'type',
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Border Radius', 'dessau-core' ),
							    'description' => esc_html__( 'Please insert border radius(Rounded corners) in px. For example: 4 ', 'dessau-core' ),
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Border Width', 'dessau-core' ),
							    'dependency' => array( 'element' => 'type', 'value'   => array( 'qodef-circle', 'qodef-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Hover Icon Color', 'dessau-core' )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Use Link as Anchor', 'dessau-core' ),
							    'value'       => array( 'Use this icon as Anchor?' => 'yes' ),
							    'description' => esc_html__( 'Check this box to use icon as anchor link (eg. #contact)', 'dessau-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = dessau_core_get_shortcode_module_template_part( 'templates/icon', 'icon', '', $params );
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'qodef-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'qodef-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . dessau_select_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'qodef-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . dessau_select_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . dessau_select_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . dessau_select_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . dessau_select_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'qodef-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . dessau_select_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
