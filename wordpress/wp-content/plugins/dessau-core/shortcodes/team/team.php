<?php
namespace DessauCore\CPT\Shortcodes\Team;
...
class Team implements lib\ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ( $x = 1; $x < 6; $x ++ ) {
			$teamIconCollections = dessau_select_icon_collections()->getCollectionsWithSocialIcons();
			foreach ( $teamIconCollections as $collection_key => $collection ) {
				
				$team_social_icons_array[] = array(
					'type'       => 'dropdown',
					'heading'    => esc_html__( 'Social Icon ', 'dessau-core' ) . $x,
			...
			$team_social_icons_array[] = array(
				'type'       => 'textfield',
				'heading'    => esc_html__( 'Social Icon ', 'dessau-core' ) . $x . esc_html__( ' Link', 'dessau-core' ),
				'param_name' => 'team_social_icon_' . $x . '_link',
				'dependency' => array(
					'element' => 'team_social_icon_pack',
					'value'   => dessau_select_icon_collections()->getIconCollectionsKeys()
				)
			);
			
			$team_social_icons_array[] = array(
				'type'       => 'dropdown',
				'heading'    => esc_html__( 'Social Icon ', 'dessau-core' ) . $x . esc_html__( ' Target', 'dessau-core' ),
				'param_name' => 'team_social_icon_' . $x . '_target',
				'value'      => array(
					esc_html__( 'Same Window', 'dessau-core' ) => '_self',
					esc_html__( 'New Window', 'dessau-core' )  => '_blank'
				),
		...
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Team', 'dessau-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array_merge(
						array(
							array(
								...
								'value'       => array(
									esc_html__( 'Info Below Image', 'dessau-core' )    => 'info-below-image',
									esc_html__( 'Info On Image Hover', 'dessau-core' ) => 'info-on-image'
								),
								'save_always' => true
							),
							array(
							...
							array(
								...
								'heading'    => esc_html__( 'Name', 'dessau-core' )
							),
							array(
								'type'        => 'dropdown',
								'param_name'  => 'team_name_tag',
								'heading'     => esc_html__( 'Name Tag', 'dessau-core' ),
							...
							array(
								...
								'heading'    => esc_html__( 'Name Color', 'dessau-core' ),
								'dependency' => array( 'element' => 'team_name', 'not_empty' => true )
							),
							array(
								'type'       => 'textfield',
								'param_name' => 'team_position',
								'heading'    => esc_html__( 'Position', 'dessau-core' )
							),
							array(
								'type'       => 'colorpicker',
								'param_name' => 'team_position_color',
								'heading'    => esc_html__( 'Position Color', 'dessau-core' ),
							...
							array(
								...
								'heading'     => esc_html__( 'Social Icon Pack', 'dessau-core' ),
								'value'       => array_merge( array( '' => '' ), dessau_select_icon_collections()->getIconCollectionsVCExclude( 'linea_icons' ) ),
								'save_always' => true
							),
						),
	...
	public function render( $atts, $content = null ) {
		...
		for ( $x = 1; $x <= $number_of_social_icons; $x ++ ) {
			...
			foreach ( dessau_select_icon_collections()->iconCollections as $collection_key => $collection ) {
		...
		$html = dessau_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'team', '', $params );
	...
	private function getTeamSocialIcons( $params ) {
		...
		if ( $team_social_icon_pack !== '' ) {
			...
			$icon_pack                    = dessau_select_icon_collections()->getIconCollection( $team_social_icon_pack );
			...
			for ( $i = 1; $i <= $params['number_of_social_icons']; $i ++ ) {
				...
				if ( $team_social_icon !== '' ) {
					...
					$social_icons[] = dessau_select_execute_shortcode( 'qodef_icon', $team_icon_params );
				}
			}
		}
