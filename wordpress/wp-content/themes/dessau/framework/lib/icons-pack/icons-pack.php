...
if ( ! function_exists( 'dessau_select_get_icon_pack_collections' ) ) {
	function dessau_select_get_icon_pack_collections() {
		$options = apply_filters( 'dessau_select_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'dessau_select_get_icon_pack_collections' );
}

if ( ! function_exists( 'dessau_select_get_icon_pack_options' ) ) {
	function dessau_select_get_icon_pack_options() {
		$options = apply_filters( 'dessau_select_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'dessau_select_get_icon_pack_options' );
}

if ( ! function_exists( 'dessau_select_get_icon_pack_social_options' ) ) {
	function dessau_select_get_icon_pack_social_options() {
		$options = apply_filters( 'dessau_select_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'dessau_select_get_icon_pack_social_options' );
}

/*
 * Class: DessauSelectIconCollections
 */
class DessauSelectIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = dessau_select_get_icon_pack_collections( true );
		$this->iconPackOptions       = dessau_select_get_icon_pack_options();
		$this->iconPackSocialOptions = dessau_select_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = dessau_select_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'dessau' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'dessau' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'dessau' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'dessau' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'dessau' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'dessau' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'dessau' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'dessau' )       => 'fa-lg',
			esc_html__( 'Small', 'dessau' )      => 'fa-2x',
			esc_html__( 'Medium', 'dessau' )     => 'fa-3x',
			esc_html__( 'Large', 'dessau' )      => 'fa-4x',
			esc_html__( 'Very Large', 'dessau' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iDessauSelectIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'dessau' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'dessau' ) ] = '';
