...
/*
   Class: DessauSelectMultipleImages
   A class that initializes Select Multiple Images
*/

class DessauSelectMultipleImages implements iDessauSelectRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$dessau_select_Framework->qodeMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="qodef-gallery-uploader">
								<a class="qodef-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'dessau' ); ?></a>
								<a class="qodef-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'dessau' ); ?></a>
							</div>
                            <?php wp_nonce_field( 'dessau-qode-update-images_' . esc_attr($this->name), 'dessau-qode-update-images_' . esc_attr($this->name) ); ?>
						</div>
					</div>
				</div>
			</div>
...
class DessauSelectTwitterFramework implements iDessauSelectRender {
	public function render( $factory ) {
		$twitterApi = DessauTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'dessau' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'dessau' ) : esc_html__( 'Connect with Twitter', 'dessau' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="qodef-page-form-section" id="qodef_enable_social_share">
			<div class="qodef-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'dessau' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'dessau' ); ?></p>
			</div>
			<div class="qodef-section-content">
				<div class="container-fluid">
					<div class="row">
...
class DessauSelectInstagramFramework implements iDessauSelectRender {
	public function render( $factory ) {
		$instagram_api = DessauInstagramApi::getInstance();
		$message       = '';
		
		//if code wasn't saved to database
		if ( ! get_option( 'dessau_instagram_code' ) ) {
			...
			if ( ! empty( $_GET['code'] ) ) {
				...
				$message = esc_html__( 'You have successfully connected with your Instagram account. If you have any issues fetching data from Instagram try reconnecting.', 'dessau' );
		...
		$buttonText = $instagram_api->hasUserConnected() ? esc_html__( 'Re-connect with Instagram', 'dessau' ) : esc_html__( 'Connect with Instagram', 'dessau' );
		?>
		<?php if ( $message !== '' ) { ?>
			<div class="alert alert-success" style="margin-top: 20px;">
				<span><?php echo esc_html( $message ); ?></span>
		...
		<div class="qodef-page-form-section" id="qodef_enable_social_share">
			<div class="qodef-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'dessau' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'dessau' ); ?></p>
			</div>
			<div class="qodef-section-content">
				<div class="container-fluid">
					<div class="row">
...
class DessauSelectRepeater implements iDessauSelectRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false) {
		...
		$this->name        = $name;
		$this->num_of_rows = 1;
		$this->button_text = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'dessau' );
			...
			<?php if ($this->table_layout) { ?>
				<div class="qodef-repeater-table-heading">
					<div class="qodef-repeater-fields-holder">
						<div class="qodef-repeater-table-cell qodef-repeater-sort"><?php esc_html_e( 'Order', 'dessau' ) ?></div>
						...
						<div class="qodef-repeater-table-cell qodef-repeater-remove"><?php esc_html_e( 'Remove', 'dessau' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="qodef-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="qodef-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = dessau_select_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class DessauSelectFieldAddress extends DessauSelectFieldType {
				...
				<div class="container-fluid">
					<div class="row">
						...
						<?php endif; ?>
							...
							<button id="find" class="btn btn-primary"><?php esc_html_e( 'Place the pin on the map', 'dessau' ); ?></button>
							<a id="reset" href="#" style="display:none;"><?php esc_html_e( 'Reset Marker', 'dessau' ); ?></a>
						</div>
					</div>
				</div>
			</div>
...
class DessauSelectFieldIcon extends DessauSelectFieldType {
    public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
        $class = '';
        ...
        $icon_packs        = dessau_select_icon_collections()->getIconCollectionsEmpty();
        $icons_collections = dessau_select_icon_collections()->getIconCollectionsKeys();
        ?>
            ...
            <div class="qodef-section-content">
                <div class="container-fluid">
                    ...
                    <?php foreach ( $icons_collections as $icons_collection ) { ?>
                        <?php
                        $icons_param = dessau_select_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
                        $style       = 'display: none';
                        ...
                        <div class="row qodef-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                            <div class="col-lg-<?php echo esc_attr($col_width); ?>">
                                <select name="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>" class="<?php echo esc_attr($select2) ?> form-control qodef-form-element">
                                    <?php
                                    $icons      = dessau_select_icon_collections()->getIconCollection( $icons_collection );
