...
if ( ! class_exists( 'DessauSelectWelcomePage' ) ) {
	class DessauSelectWelcomePage {
		...
		/**
		 * Get the instance of DessauSelectWelcomePage
		 *
		 * @return self
		 */
		...
		private function __construct() {
			...
			add_action( 'dessau_select_admin_scripts_init', array( $this, 'enqueueStyles' ) );
		}
		...
		function initActivationHook() {
			...
			if ( ! is_network_admin() ) {
				set_transient( '_dessau_select_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcomePageRedirect() {
			...
			if ( ! get_transient( '_dessau_select_welcome_page_redirect' ) ) {
				return;
			}
			
			// Delete the redirect transient
			delete_transient( '_dessau_select_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'dessau_select_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function addWelcomePage() {
			...
			add_theme_page(
				esc_html__( 'About', 'dessau' ),
				esc_html__( 'About', 'dessau' ),
				current_user_can( 'edit_theme_options' ),
				'dessau_select_welcome_page',
				array( $this, 'welcomePageContent' )
			);
			
			remove_submenu_page( 'themes.php', 'dessau_select_welcome_page' );
		}
			...
			<div class="wrap about-wrap qodef-welcome-page <?php echo esc_attr( $qodef_welcome_page_class ); ?>">
				<div class="qodef-welcome-page-content">
					<div class="qodef-welcome-page-logo">
						<img src="<?php echo esc_url( dessau_select_get_skin_uri() . '/assets/img/logo.png' ); ?>" alt="<?php esc_attr_e( 'Profile Logo', 'dessau' ); ?>" />
					</div>
					<h1 class="qodef-welcome-page-title">
						<?php echo sprintf( esc_html__( 'Welcome to %s', 'dessau' ), $qodef_theme_name ); ?>
					...
					<div class="about-text qodef-welcome-page-text">
						...
						<img src="<?php echo esc_url( $qodef_theme_screenshot ); ?>" alt="<?php esc_attr_e( 'Theme Screenshot', 'dessau' ); ?>" />
						
						<h4><?php esc_html_e( 'Useful Links:', 'dessau' ); ?></h4>
						<ul class="qodef-welcome-page-links">
							<li>
								<a href="<?php echo sprintf('https://%s.ticksy.com/', SELECT_PROFILE_SLUG ); ?>" target="_blank"><?php esc_html_e( 'Support Forum', 'dessau' ); ?></a>
							</li>
							<li>
							...
							<li>
								<a href="<?php echo sprintf('https://themeforest.net/user/%s-themes/portfolio/', SELECT_PROFILE_SLUG ); ?>" target="_blank"><?php esc_html_e( 'All Our Themes', 'dessau' ); ?></a>
							</li>
							<li>
		...
		function enqueueStyles() {
			wp_enqueue_style( 'dessau-select-welcome-page-style', SELECT_FRAMEWORK_ADMIN_ASSETS_ROOT . '/css/qodef-welcome-page.css' );
		}
	}
}

DessauSelectWelcomePage::getInstance();